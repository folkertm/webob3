...
def test_unicode_cookies_error_raised():
    res = Response()
    with pytest.raises(ValueError):
        Response.set_cookie(res, "x", text_(b"\\N{BLACK SQUARE}", "unicode_escape"))
...
def test_content_length():
    ...
    r4 = Response(app_iter=[b"x"] * 10, content_length=20)  # wrong content_length
...
def test_app_iter_range():
    ...
    for app_iter in [
        ...
        r = Response(app_iter=app_iter, content_length=6, conditional_response=True)
...
def test_from_file_w_leading_space_in_header():
    ...
    res1 = Response()
    file_w_space = io.BytesIO(b"200 OK\n\tContent-Type: text/html; charset=UTF-8")
...
def test_request_uri_no_script_name():
    ...
    environ = {"wsgi.url_scheme": "http", "HTTP_HOST": "test.com"}
...
def test_app_iter_del():
    res = Response(content_length=3, app_iter=["123"])
    del res.app_iter
    assert res.body == b""
    assert res.content_length is None
...
def test_raises_none_charset():
    with pytest.raises(TypeError):
        Response(content_type="image/jpeg", body=text_(b"test"), charset=None)
